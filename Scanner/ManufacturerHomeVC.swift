...
class ManufacturerHomeVC: HomeVC {
    ...
    override func viewDidLayoutSubviews() {
        ...
        guard !didLoadFirstPage, tableView.bounds.height > 0
        else {
            return
        }
        guard let compAcc = try? EthereumAddress(hex: Config.companyAccountAddress, eip55: true)
        else {
            return
        }
        let vm = ProductListViewModel(source: .manufacturer(compAcc), web3: web3)
        vm.onChanged = {
            [weak self] in self?.handleVMChanged()
        }
        self.vm = vm
    ...
    @objc private func addProduct() {
        let vc = AddProductInfoVC()
        vc.onFinished = { [weak self] in self?.refresh() }
...
extension ManufacturerHomeVC: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        guard let vm, indexPath.row == vm.products.count - 1, vm.hasMore
        else {
            return
        }
        vm.loadPage(limit: pageLimit)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        vc.role = role
        vc.onStatusChanged = {
            [weak self] productId, newStatus in
    ...
    func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        guard role == .manufacturer_emp, editingStyle == .delete
        else {
            return
        }

        let alert = UIAlertController(title: "Delete product", message: "This action cannot be undone. Continue?",preferredStyle: .alert)
        ...
        alert.addAction(UIAlertAction(title: "Delete", style: .destructive) { [weak self] _ in
            ...
            let progress = self.makeProgressAlert(title: "Deleting product", message: "Please wait…")
            self.present(progress, animated: true)

            Task {
                do {
                    try await self.vm?.deleteProduct(at: indexPath.row)
                    ...
                    print("=== ERROR ===\nfull: \(error)")
                    progress.dismiss(animated: true) {
                        let fail = UIAlertController(title: "Transaction failed", message: error.localizedDescription, preferredStyle: .alert)
