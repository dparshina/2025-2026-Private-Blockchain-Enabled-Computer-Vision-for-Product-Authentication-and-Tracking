...
final class WithdrawDepositVC: UIViewController {
    ...
    private func setupLabels() {
        titleLabel.text = "Withdraw"
        titleLabel.font = .systemFont(ofSize: 28, weight: .bold)
        titleLabel.textColor = .label

        explanationLabel.text = "Move ETH from your company's account back to your connected wallet address. Enter the amount you want to withdraw, it will be sent to the same wallet you used to sign in."
    ...
    private func setupConfirmButton() {
        confirmButton.setTitle("Confirm withdrawal", for: .normal)
        confirmButton.setTitleColor(.white, for: .normal)
        confirmButton.titleLabel?.font = .systemFont(ofSize: 16, weight: .semibold)
        confirmButton.setImage(UIImage(systemName: "arrow.up"), for: .normal)
    ...
    private func handleConfirm() {
        ...
        let to = web3.account?.hex(eip55: false) ?? ""
        guard !to.isEmpty else {
            presentDepositAlert(title: "Wallet not connected", message: "Connect your wallet before withdrawing.")
            return
        }
        ...
        Task.detached { [weak self] in
            ...
            do {
                let balance = try await web3.getDeposit()
                guard wei <= balance else {
                    await MainActor.run {
                        self.setConfirmEnabled(true)
                        self.presentDepositAlert(
                            title: "Insufficient balance",
                            message: "You're trying to withdraw \(formatWei(wei)) SEP but the account holds \(formatWei(balance)) SEP."
                        )
                    }
                    return
                }

                _ = try await web3.withdrawDeposit(withdrawTo: to, money: wei)
                ...
                await MainActor.run {
                    self.setConfirmEnabled(true)
                    self.presentDepositAlert(title: "Withdrawal failed", message: error.localizedDescription)
                }
            }
        }
    }
